...
export default function DocumentsPage() {
  ...
  async function fillRicettaPrescrizioni(
    ...
    if (patient) {
      p1.drawText(`${patient.name || ""} ${patient.surname || ""}`, {
        x: 182, y: H - 215, size: fs, font, color: black
      })
    }
  ...
  async function fillTabellaMedicazioni(
    ...
    if (patient) {
      p1.drawText(`${patient.name || ""} ${patient.surname || ""}`, {
        x: 132, y: H - 86, size: fs, font, color: black
      })
    }
  }
  ...
  async function fillChirurgiaAmbulatoriale(
    ...
    if (patient) {
      p1.drawText(`${patient.name || ""} ${patient.surname || ""}`, {
        x: 145, y: H - 155, size: fs, font, color: black
      })
    }

    // Firma Medico in basso
    const sw = 100, sh = 18
    if (docB) p1.drawImage(await embedSig(pdfDoc, docB), { x: 380, y: 170, width: sw, height: sh })
  }
